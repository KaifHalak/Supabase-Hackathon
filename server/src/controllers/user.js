...
export async function getUserStats(req, res) {
	try {
		const { data: userArr, error: uError } = await supabase.from("Users").select("*").eq("email", req.user.email).eq("username", req.user.name);
		...
		console.log(`User with ID ${user.userId} is in position ${userRank + 1}`);
...
export async function insertUserToDB({ sub, name, picture, email }) {
	const exists = await supabase.from("Users").select().eq("email", email).eq("username", name);
